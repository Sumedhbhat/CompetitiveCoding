  ...
  public:
	int minExtraChar(string s, vector<string> &dictionary) {
		unordered_set<string> dict(dictionary.begin(),
		                           dictionary.end());  // Set for O(1) lookup
		int n = s.size();
		vector<int> dp(
		    n + 1, n);  // DP array initialized with maximum extra characters
		...
		for (int i = 1; i <= n; i++) {
			...
			for (int j = 0; j < i; j++) {
				...
				if (dict.find(sub) != dict.end()) {
					dp[i] = min(dp[i],
					            dp[j]);  // If substring is found in dictionary
				}
			}
			dp[i] =
			    min(dp[i],
			        dp[i - 1] + 1);  // Consider the current character as extra
		}
